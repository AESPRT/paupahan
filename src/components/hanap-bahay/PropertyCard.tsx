...
// ✅ A reliable, free apartment stock photo from Unsplash
...
export function PropertyCard({ property, onViewDetails }: PropertyCardProps) {
  ...
  const [imgSrc, setImgSrc] = useState(
    property.coverImage || FALLBACK_IMAGE, // ✅ use fallback immediately if empty
  );
    ...
    >
      ...
      <div className="relative h-72 w-full overflow-hidden bg-[var(--forest-deep)]/10">
        <Image
          ...
          onError={() => setImgSrc(FALLBACK_IMAGE)} // ✅ fallback if URL is broken
        />

        {/* rest of your gradients, badges, buttons — unchanged */}
        ...
        <div className="absolute top-4 right-4 flex items-center gap-2">
          ...
          <button
            ...
            className={`flex h-10 w-10 items-center justify-center rounded-full bg-white/95 backdrop-blur-md transition-all duration-300 hover:scale-110 shadow-lg ${
              isBookmarked
                ? "text-[var(--marigold-deep)] bg-white"
                : "text-[var(--ink)]"
            }`}
      ...
      <div className="flex flex-1 flex-col p-6">
        ...
        <div className="flex flex-wrap gap-2 mb-6">
          {property.amenities.slice(0, 3).map((amenity, idx) => (
            <span
              key={idx}
              className="rounded-xl bg-[var(--paper)] border border-[var(--line)] px-3 py-1 text-xs font-bold text-[var(--ink)]"
            >
              {amenity}
            </span>
          ))}
        ...
        <div className="mt-auto flex items-center justify-between border-t border-[var(--line)] pt-5">
          <div>
            <span className="text-[10px] font-extrabold uppercase tracking-widest text-[var(--muted)] block mb-0.5">
              Renta Kada Buwan
            </span>
