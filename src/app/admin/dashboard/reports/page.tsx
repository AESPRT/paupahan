...
import { getAdminReportsData, generateReportAction } from "@/src/actions/admin-reports";

// Mga Kategorya na may SVG icons sa halip na emoji
...
export default function ReportsPage() {
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      if (res.success && res.financialSummary) {
  ...
  const handleDownload = (reportId: string, format: ExportFormat) => {
    startTransition(async () => {
      ...
      if (res.success && res.fileData && res.fileName) {
        // I-trigger ang pag-download ng file sa browser gamit ang Blob
  ...
  const handleGenerateAll = () => {
    startTransition(async () => {
      alert("Ibinabalot ang lahat ng ulat... Mangyaring maghintay sandali.");
    });
  };
